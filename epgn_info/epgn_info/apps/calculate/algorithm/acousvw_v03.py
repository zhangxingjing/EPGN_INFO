...
def rpmSelect2(raw_time, raw_rpm, rpm_step, rpmtype):
    if rpmtype == 'falling':
        raw_rpm = raw_rpm[::-1]
    fs = detectFs(raw_time)
...
def octave_fft(raw_data, spectrum_size, fs, overlap, weighting):
    (fl, db) = fft_average(raw_data, spectrum_size, fs, overlap, weighting)
...
def figure_fft_rpm(raw_time, raw_data, raw_rpm, rpm_step, rpmtype, spectrum_size, weighting):
    fs = detectFs(raw_time)
    rpm_step = 10
    spectrum_size = 16384
    fs = 44100
...
def figure_octave_fft(raw_time, raw_data, spectrum_size, overlap, weighting):
    fs = detectFs(raw_time)
    (fc, db) = octave_fft(raw_data, spectrum_size, fs, overlap, weighting)
    plt.figure()
...
def figure_order_vfft(raw_time, raw_data, raw_rpm, rpm_step, rpmtype, order, orderResolution, orderWidth, smoothFrac):
    fs = detectFs(raw_time)
    rpm_step = 10
    order = 2
    orderResolution = 0.5
...
def figure_level_rpm(raw_time, raw_data, raw_rpm, rpm_step, rpmtype, timeWeighting, A):
